...
void setPin(unsigned char address, unsigned char registeri, unsigned char value) {
    i2c_master_start();
    i2c_master_send(address << 1);
    i2c_master_send(registeri);
    i2c_master_send(value);
    i2c_master_stop();
}

unsigned char readPin(unsigned char address, unsigned char registeri) {
    unsigned char x;
    i2c_master_start();
    i2c_master_send(address << 1);
    i2c_master_send(registeri);
...
void i2c_read_multiple(unsigned char address, unsigned char registeri, unsigned char * data, int length) {
    unsigned char x;
    int i = 0;
    i2c_master_start();
    i2c_master_send(address << 1);
